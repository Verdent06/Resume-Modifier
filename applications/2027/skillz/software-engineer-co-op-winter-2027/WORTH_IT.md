...
1. **Backend SWE screen is a match without inventing Java/Go.** JD tests server-side languages (Java, Go, **or similar**), REST APIs, microservices, Git/CI, AWS preferred, CS fundamentals. Pool has Python + TypeScript through use, Flask/FastAPI REST, Docker/Celery pipelines, GitHub Actions, AWS EC2/S3. Java and Go are not in inventory — do not invent them. "Or similar" is the same pattern as Java at Mastercard / BNY / Fifth Third.

2. **Payment / deposit-withdrawal analog is carryable, not fabricated.** MDC sole-engineer Flask REST on AWS EC2 + Requests/Pandas ETL, Vylet injection-safe SQL freshness + Dockerized pipeline, SignalWeaver FastAPI/React — shipped APIs and data-integrity work. Do **not** claim payment rails, PCI, card processing, Java/Go payment services, or Skillz platform experience.

3. **Production debugging + tests exist.** CaseStudyPrep S3 presigned-URL recovery (27% upload failures); Vylet name-collision defect (79% → 89%). JD asks for production issue work on deposit/withdrawal/payout — narrate those as analog, not as payments.
